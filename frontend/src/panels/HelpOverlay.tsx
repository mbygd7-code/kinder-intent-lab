  ...
  return (
    <div className="help-doc">
      ...
      <p className="help-note">
        ...
        지금은 원하는 일 하나에 메뉴를 찾아 여러 단계를 눌러야 하지만, 이 뇌가 다 자라면{' '}
        <strong>평소 말투 한마디</strong> — "아까 그 사진 부모님들께 보내줘" — 로 끝나요.
      </p>
      <table className="help-table">
        <tbody>
          <tr>
            <td>⏱ 시간</td>
            <td>메뉴 찾기·반복 클릭이 한마디로 — 하루 수십 번의 잔업이 줄고, 그 시간은{' '}
              <b>아이들 곁으로</b> 돌아가요</td>
          </tr>
          <tr>
            <td>🙋 내게 맞춤</td>
            <td>쓸수록 <b>내 말버릇</b>에 맞춰져요 (영향엔 상한선 — 치우치지 않게)</td>
          </tr>
          <tr>
            <td>🛡 실수 방지</td>
            <td>학부모 전송·출결·자료 삭제 같은 <b>위험한 7가지</b>는 확신이 없으면 실행하지
              않고 되묻게 설계 — 실수 대신 확인</td>
          </tr>
          <tr>
            <td>⭐ 내 기여</td>
            <td>내 답 하나하나가 뇌의 교과서 — 한 번 가르치면 <b>모든 선생님</b>의 킨더버스가
              똑똑해져요</td>
          </tr>
        </tbody>
      </table>
      <p>
        <strong>타 서비스가 따라올 수 없는 이유</strong> — 범용 AI 챗봇을 붙인 서비스는
        유아교육 현장의 짧고 애매한 말("아까 그거", "이거 좀")을 모르고, 자기가 얼마나
        정확한지 <strong>증명하지 못하고</strong>, 위험한 순간에도 자신만만하게 틀려요.
        킨더브레인은 ① 실제 선생님들이 가르친 <strong>현장 데이터</strong>(돈 주고도 못 사는
        자산), ② 전문가 시험으로 <strong>정확도를 숫자로 증명</strong>(목표 80%), ③ 위험한
        행동엔 <strong>별도 안전 검사</strong>까지 — "믿고 맡겨도 된다는 증거"를 가진 AI는
        이 데이터와 시험 없이는 못 만들어요. 그리고 그 데이터의 원천이 바로{' '}
        <strong>선생님의 답</strong>입니다.
      </p>
      <p className="help-note">
        🌱 뇌는 저절로 똑똑해지지 않아요. 컴퓨터 연습문제는 뼈대일 뿐, 진짜 교과서는
        선생님의 답뿐이에요. <strong>오늘의 5분 강화하기가 내일의 몇 시간</strong>이 되어
        모두에게 돌아옵니다.
      </p>
