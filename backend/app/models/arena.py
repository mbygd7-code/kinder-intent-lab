...
class KtibUpload(Base):
    """시험지 업로드 이력 — 등록(commit)마다 한 줄 + 올린 원문 보관(마이그레이션 0018).

    사람이 목록에서 보고 원문(source_document)을 다시 여는 감사·회수용 표.
    episodes/tier/label/brightness와 무관하다(이력을 지워도 채점 축은 불변).
    """

    __tablename__ = "ktib_uploads"
    __table_args__ = (Index("idx_ktib_uploads_created", "created_at"),)
    ...
    reviewers = mapped_column(JSONB, nullable=False, server_default=text("'[]'::jsonb"))
    ...
    skipped_duplicate: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default=text("0")
    )
    # 배치 요약 — kappa는 O/X 쏠림에 퇴화할 수 있어 일치율과 함께 보관(§3-3 v1.6)
    ...
    eligible_total: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("0"))
    # 올린 원문 그대로(CSV/YAML) — 목록에서 열어 확인·회수용. 전문가 저작이라 원문 보관 허용(§1-S2 대상 아님).
