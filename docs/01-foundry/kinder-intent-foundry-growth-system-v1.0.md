...
| `GOLD` | **인간 검수 2인 이상 일치**(일치도 기록 — 아래 인증 척도). 확정 라벨 보유 |

> 참고: tier에 `SYNTHETIC`을 두지 않는다 — 출생 정보는 `origin_channel` 축이 담당하며, 같은 개념을 두 축에 두면 축 분리 원칙이 무너진다.

> **GOLD 신뢰도 인증 척도 (v1.6):** 두 검수자의 "일치"는 검수 방식에 맞는 척도로 인증한다. **독립 라벨링**(검수자가 여러 의도 중 하나를 스스로 고름 — blind 2차 검수)은 우연 보정이 유효하므로 `agreement_kappa ≥ review.min_agreement_kappa`. **O/X 승인 검수**(저작된 문항이 라벨에 맞는지 O/X만 체크)는 판정이 O로 쏠려 Cohen's kappa가 base-rate 역설로 퇴화하므로(관측 일치 95.7%인데 kappa가 음수인 실측 사례) 관측 일치율 `agreement_rate ≥ review.min_expert_agreement`로 인증한다. **둘 중 하나**를 넘으면 GOLD, 둘 다 미기록이면 거부(측정 못 한 일치는 일치가 아니다). 두 요건(검수자 2인 이상, 비합성 채널)과 §3-4 DB CHECK는 불변이다.
