...
컨테이너는 기동 시 **`alembic upgrade head`(마이그레이션) 후 uvicorn**을 실행합니다. 헬스체크는
...
> **다중 인스턴스 주의:** 컨테이너가 기동 때마다 마이그레이션을 돌립니다. 인스턴스를 2개 이상으로
> 스케일할 계획이면 마이그레이션을 별도 릴리즈/잡 단계로 빼고, 컨테이너 CMD에서는 uvicorn만
> 실행하도록 나누세요(동시 마이그레이션 경합 방지).
